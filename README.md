...
AiDrop-Core is an MVP platform that analyzes blockchain user on-chain data using AI. AiDrop's AI model connects to a selected blockchain, collects on-chain data for user addresses, applies unsupervised learning (clustering) and supervised learning (classification) models to profile users, and provides analysis results through a FastAPI-based API server.
It also includes an aiAgent that creates user profile images based on user profiling, and stores the generated profile images in S3.
...
AiDrop-Core는 블록체인 사용자 온체인 데이터를 AI로 분석하는 MVP 플랫폼입니다. AiDrop의 ai 모델은 선택된 블록체인에 연결하여 사용자 주소의 온체인 데이터를 수집하고, 비지도 학습(클러스터링)과 지도 학습(분류) 모델을 적용하여 사용자를 프로파일링한 후, FastAPI 기반 API 서버를 통해 분석 결과를 제공합니다.
사용자 프로파일링을 토대로 유저 프로필 이미지를 제작하는 aiAgent도 포함하여, 생성한 프로필이미지를 S3에 저장합니다.
